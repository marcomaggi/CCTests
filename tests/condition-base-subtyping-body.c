/*
  Part of: CCTests
  Contents: body definitions of a subtype of "base"
  Date: Dec  6, 2017

  Abstract

	Body definitions of a subtype of "base".
...
#include <cctests.h>
#include "condition-base-subtyping-header.h"
#include <stdio.h>
#include <stdlib.h>
...
static cce_condition_delete_fun_t		my_condition_delete_base_subtype;
static cce_condition_final_fun_t		my_condition_final_base_subtype;
static cce_condition_static_message_fun_t	my_condition_static_message_base_subtype;

static my_descriptor_base_subtype_t my_descriptor_base_subtype_stru = {
  /* This  "parent" field  is  set below  by  the module  initialisation
     function. */
  .descriptor.parent		= NULL,
  .descriptor.delete		= my_condition_delete_base_subtype,
  .descriptor.final		= my_condition_final_base_subtype,
  .descriptor.static_message	= my_condition_static_message_base_subtype
};

my_descriptor_base_subtype_t const * const my_descriptor_base_subtype_ptr = &my_descriptor_base_subtype_stru;
...
void
my_condition_final_base_subtype (cce_condition_t * _C)
/* Finalisation  functions are  called automatically  when the  function
   ...
   the parent type. */
{
  my_condition_base_subtype_t * C = (my_condition_base_subtype_t *) _C;
  *(C->data) = 0;
  free(C->data);
...
void
my_condition_delete_base_subtype (cce_condition_t * _C)
...
char const *
my_condition_static_message_base_subtype (cce_condition_t const * C CCE_UNUSED)
{
  return "CCTests base subtype exceptional condition";
}
...
void
my_condition_init_base_subtype (cce_destination_t L, my_condition_base_subtype_t * C, int the_data)
...
my_condition_new_base_subtype (cce_destination_t upper_L, int the_data)
/* This constructor function is the  public interface to the constructor
 * of condition objects of type "my_condition_base_subtype_t".
 *
 * Here we:
 *
...
{
  my_descriptor_base_subtype_stru.descriptor.parent = &(cctests_descriptor_base_ptr->descriptor);
}
