/*
  Part of: CCTests
  Contents: test for subtyping of "test signal-2" conditions
  Date: Dec  6, 2017

  Abstract

	Test file for subtyping of "test signal-2" conditions.
...
main (void)
{
  ...
  condition_signal_2_subtyping_init_module();

  {
    ...
    if (cce_location(L)) {
      ...
      if (my_condition_is_signal_2_subtype(cce_condition(L))) {
	CCLIB_PC(my_condition_signal_2_subtype_t, C, cce_condition(L));
	fprintf(stderr, "%s: is signal-2 subtype, data=%d\n", __func__, *(C->data));
      } else {
	...
	fprintf(stderr, "%s: is CCTests test signal-2 condition\n", __func__);
      } else {
	fprintf(stderr, "%s: wrong condition-object type\n", __func__);
	exit(EXIT_FAILURE);
      }
	...
	exit(EXIT_FAILURE);
      ...
      cce_raise(L, my_condition_new_signal_2_subtype(L, 123));
