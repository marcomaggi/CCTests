...
cctests_decl my_descriptor_signal_subtype_t const * const	my_descriptor_signal_subtype_ptr;

cctests_decl void my_condition_init_signal_subtype (cce_destination_t L, my_condition_signal_subtype_t * C, int the_data)
  __attribute__((__nonnull__(1,2)));

cctests_decl cce_condition_t const * my_condition_new_signal_subtype (cce_destination_t L, int the_data)
...
{
  return cce_is_condition(C, &(my_descriptor_signal_subtype_ptr->descriptor));
}

cctests_decl void condition_signal_subtyping_init_module (void);
