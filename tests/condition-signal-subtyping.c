/*
  Part of: CCTests
  Contents: test for subtyping of "test signal" conditions
  Date: Dec  6, 2017

  Abstract

	Test file for subtyping of "test signal" conditions.
...
main (void)
{
...

  {
    ...
    if (cce_location(L)) {
      ...
      if (my_condition_is_signal_subtype(cce_condition(L))) {
	CCLIB_PC(my_condition_signal_subtype_t, C, cce_condition(L));
	fprintf(stderr, "%s: is signal subtype, data=%d\n", __func__, *(C->data));
      } else {
	...
	exit(EXIT_FAILURE);
      ...
      cce_raise(L, my_condition_new_signal_subtype(L, 123));
